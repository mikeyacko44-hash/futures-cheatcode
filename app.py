"""
Futures Cheat Code — Production Trade Desk
Clean · Mobile-first · One clear action
"""
...
.block-container { padding: 0.75rem 1rem 2rem; max-width: 720px; margin: 0 auto; }
...
.stTabs [data-baseweb="tab"] { border-radius: 9px; padding: 8px 14px; color: #8b93a7; font-weight: 500; }
...
def candle_fig(df, o8, rh, rl):
    ...
    fig.add_trace(go.Candlestick(
        ...
        whiskerwidth=0.6, name="NQ"))
    if o8:
        fig.add_hline(y=o8, line_dash="dash", line_color="#ffc107", line_width=1.2,
                      annotation_text="8PM", annotation_position="top left",
    ...
    fig.update_layout(
        height=320, margin=dict(l=0, r=8, t=8, b=0),
...
tab_desk, tab_engine, tab_ai, tab_mag, tab_alert = st.tabs(["Trade Desk", "Engine", "AI", "Mag7", "Alerts"])
...
with tab_desk:
    ...
    st.markdown(f"""<div class="kpi-grid">
      <div class="kpi"><div class="label">NQ</div><div class="value">{px_}</div><div class="sub">{dist}</div></div>
      <div class="kpi"><div class="label">Bias</div><div class="value">{ctx['bias']}</div><div class="sub">vs 8PM open</div></div>
      <div class="kpi"><div class="label">Mag7</div><div class="value">{ctx['mag']['label'].replace('STRONG ','')}</div><div class="sub">{ctx['mag'].get('bullish',0)}/{ctx['mag'].get('total',7)} bull</div></div>
    ...
    st.markdown(asia_chip + hunt_chip, unsafe_allow_html=True)
    ...
    if rows:
        ...
        st.caption("Levels build after Asia session data is available.")

    st.markdown('<div class="sec">NQ · 5 minute</div>', unsafe_allow_html=True)
    if len(df) > 5:
        st.plotly_chart(candle_fig(df, ctx["o8"], ctx["rh"], ctx["rl"]), use_container_width=True, config={"displayModeBar": False})
    else:
...
with tab_engine:
    ...
    try:
        ...
        a.metric("Equity", f"${state.get('equity', 0):,.0f}")
        b.metric("Drawdown", f"${state.get('drawdown_usd', 0):,.0f}")
        c.metric("Risk / trade", f"${risk['risk_usd']}")
        d.metric("Position", state.get("open_direction") or "Flat")
        if result.get("action") in ("ENTERED", "EXITED"):
            st.success(f"{result['action']}: {result.get('direction')} · {result.get('entry', result.get('exit'))}")
        ...
        st.markdown('<div class="sec">Self-score</div>', unsafe_allow_html=True)
        s1,s2,s3,s4 = st.columns(4)
        s1.metric("Trades", stats["total_trades"])
        s2.metric("Win rate", f"{stats['win_rate']}%")
        s3.metric("Avg points", stats["avg_points"])
        s4.metric("Total PnL", f"${stats['total_pnl_usd']}")
...
with tab_ai:
    ...
    if "chat" not in st.session_state:
        st.session_state.chat = [{"role": "assistant", "content": "Ask me: long or short? levels? risk size? Asia status?"}]
...
with tab_mag:
    ...
    m = ctx["mag"]
    st.metric("Read", m["label"])
    st.caption(f"{m.get('bullish', 0)} of {m.get('total', 7)} bullish today")
    if not mag7.empty:
        st.dataframe(mag7, use_container_width=True, hide_index=True)

with tab_alert:
    st.markdown('<div class="sec">Phone alerts</div>', unsafe_allow_html=True)
    st.write("In Streamlit → **Manage app → Settings → Secrets**, paste:")
    st.code('TELEGRAM_BOT_TOKEN = "your_token"\nTELEGRAM_CHAT_ID = "your_chat_id"', language="toml")
    st.caption("Engine ENTER / EXIT pushes to Telegram when secrets are set.")
    if st.button("Send test alert"):
        ok = test_alert()
        st.success("Sent to Telegram") if ok else st.error("Not configured or failed")

st.caption("Data · Yahoo Finance NQ=F (slight delay) · Refresh 30s · Not financial advice")
